 ...
 * 強化版参照検出器 v3.5.0
 * 
 * 規則・条例・告示・通達等を含む全法令種別対応の高精度参照検出
 * v3.4.0からの改善点：
 * - 規則・条例パターンの精度向上
 * - 告示・通達・通知パターンの追加
 * - 府令・内閣府令・勅令の対応
...
export class EnhancedReferenceDetectorV35 {
  ...
  private collectAllCandidates(text: string, currentArticle?: string): ReferenceCandidate[] {
    ...
    candidates.push(...this.collectLawTypeCandidates(text)); // 改善版
          ...
          if (lawName && !this.lawNameToId.has(lawName)) {
            if (articleNum) {
              ...
              candidates.push({
                ...
                metadata: { 
                  lawType: typeName,
                  number: numberNum || undefined
                },
                startPos: match.index,
                endPos: match.index + match[0].length,
                priority: 9
              });
            } else if (numberNum || match[0].includes('号')) {
              // 号数のみの参照
              candidates.push({
                type: 'external',
                sourceText: match[0],
