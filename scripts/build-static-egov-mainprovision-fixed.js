...
class EGovMainProvisionStaticSiteGenerator {
  ...
  extractMainProvisionArticles(xmlContent) {
    ...
    // 構造情報を含めて条文を抽出
    const structureInfo = this.extractStructureWithArticles(mainProvision);
    ...
    for (const match of articleMatches) {
      ...
      // ArticleTitleの内容が条文タイトルになっている場合の処理
      const kanjiNum = this.convertArticleNumToKanji(articleNum);
      if (articleTitle === kanjiNum || articleTitle === `（${kanjiNum}）`) {
        articleTitle = '';
      }
      ...
      // 構造情報を追加
      const structureContext = this.findStructureContext(articleNum, structureInfo);
      ...
      articles.push({
        ...
        section: structureContext.section
      });
    }
  ...
  extractStructureWithArticles(mainProvision) {
    const structureInfo = [];
    
    // 各構造要素の開始位置と終了位置を記録
    const partRegex = /<Part[^>]*Num="([^"]+)"[^>]*>([\s\S]*?)<\/Part>/g;
    const chapterRegex = /<Chapter[^>]*Num="([^"]+)"[^>]*>([\s\S]*?)<\/Chapter>/g;
    const sectionRegex = /<Section[^>]*Num="([^"]+)"[^>]*>([\s\S]*?)<\/Section>/g;
    
    // 編の情報を抽出
    let partMatch;
    while ((partMatch = partRegex.exec(mainProvision)) !== null) {
      ...
      // この編内の条文番号を特定
      const articleNums = [];
      const articleMatches = partContent.matchAll(/<Article[^>]*Num="([^"]+)"/g);
      for (const match of articleMatches) {
        articleNums.push(match[1]);
      }
      ...
      structureInfo.push({
        ...
        num: partNum,
        title: partTitle,
        articles: articleNums,
        startPos: partMatch.index,
        endPos: partMatch.index + partMatch[0].length
      });
    }
    
    // 章の情報を抽出
    let chapterMatch;
    while ((chapterMatch = chapterRegex.exec(mainProvision)) !== null) {
      const chapterNum = chapterMatch[1];
      const chapterContent = chapterMatch[2];
      const titleMatch = chapterContent.match(/<ChapterTitle>([^<]+)<\/ChapterTitle>/);
      const chapterTitle = titleMatch ? titleMatch[1] : '';
      
      // この章内の条文番号を特定
      const articleNums = [];
      const articleMatches = chapterContent.matchAll(/<Article[^>]*Num="([^"]+)"/g);
      for (const match of articleMatches) {
        articleNums.push(match[1]);
      }
  ...
  findStructureContext(articleNum, structureInfo) {
    let context = {
      part: null,
      chapter: null,
      section: null
    ...
    for (const struct of structureInfo) {
      if (struct.articles.includes(articleNum)) {
        if (struct.type === 'part') {
          context.part = { num: struct.num, title: struct.title };
        } else if (struct.type === 'chapter') {
          context.chapter = { num: struct.num, title: struct.title };
        }
      }
    }
...
<body>
  ...
  renderArticlesWithStructure(lawData, references) {
    ...
    for (const article of lawData.articles) {
      // 編ヘッダーの追加
      if (article.part && (!currentPart || currentPart.num !== article.part.num)) {
        html.push(`<div class="structure-header part-header">${article.part.title}</div>`);
        currentPart = article.part;
        currentChapter = null;
      }
      ...
      // 章ヘッダーの追加
      if (article.chapter && (!currentChapter || currentChapter.num !== article.chapter.num)) {
        html.push(`<div class="structure-header chapter-header">${article.chapter.title}</div>`);
        currentChapter = article.chapter;
      }
