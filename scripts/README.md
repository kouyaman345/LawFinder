...
| ファイル                           | 役割         | 説明                                       |
| ---------------------------------- | ------------ | ------------------------------------------ |
| **lawfinder-cli.ts**               | 統合 CLI     | すべての機能を統合した単一の CLI ツール    |
| **reference-manager.ts**           | 参照管理     | CLAUDE.md 推奨の参照管理システム           |
| **reference-detector-ultimate.ts** | 検出エンジン | 99.5%精度を目指す究極の 3 段階検出システム |
| **neo4j-visualization-guide.ts**   | ドキュメント | Neo4j ビジュアライゼーションガイド         |

## 🚀 lawfinder-cli.ts の機能
...
## 🧠 reference-detector-ultimate.ts の特徴
...
- **lawfinder-cli.ts**: すべての操作の統一エントリポイント（これ以上の統合は不要）
- **reference-manager.ts**: CLAUDE.md 推奨のため独立保持
- **reference-detector-ultimate.ts**: 検出エンジンとして独立性が必要
- **neo4j-visualization-guide.ts**: ドキュメントのため分離
...
1. **lawfinder-cli.ts** に新しいサブコマンドを追加
2. 必要に応じて **reference-detector-ultimate.ts** を拡張
3. **reference-manager.ts** でバージョン管理
