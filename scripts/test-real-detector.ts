...
async function testRealDetector() {
  ...
  try {
    ...
    for (const testCase of testCases) {
      ...
      // detectReferencesメソッドを呼び出し（正しいメソッド名）
    ...
    if (f1 < 90) {
      ...
      const kanjiRefs = await detector.detectReferences(kanjiTest!.text);
      ...
      const structRefs = await detector.detectReferences(structTest!.text);
      ...
      const contextRefs = await detector.detectReferences(contextTest!.text);
