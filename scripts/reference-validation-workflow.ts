...
`);
  ...
  private async syncToNeo4j(lawId: string): Promise<void> {
    const { stdout, stderr } = await execAsync(
      `npx tsx /home/coffee/projects/LawFinder/scripts/sync-to-neo4j.ts ${lawId}`
    );
  ...
  private async generateHTML(lawId: string): Promise<string> {
    ...
    try {
      await execAsync('curl -s http://localhost:3000 > /dev/null');
    } catch {
    ...
    const { stdout } = await execAsync(
      `curl -s http://localhost:3000/laws/${lawId} > ${htmlPath}`
    );
...
if (require.main === module) {
  ...
  if (command === 'validate') {
    ...
    processor.processFeedback(lawId)
      .then(() => {
        console.log('\n✅ フィードバック処理が完了しました');
      })
      .catch(error => {
